...
function ViewContent() {
  ...
  useEffect(() => {
    ...
    // Fetch the content from the public open endpoint
    fetch(`/api/open?id=${id}`)
      .then((res) => {
        if (!res.ok) {
  ...
  const { processedContent, chapters } = useMemo(() => {
    ...
    const htmlContent = htmlWithPreChapters.replace(/<p\b[^>]*>([\s\S]*?)<\/p>/gi, (match, pContent) => {
      // SECURITY NOTE: textContent is used only for regex matching, never rendered.
      // Actual rendering goes through DOMPurify sanitization below.
      // Extra replace ensures any remaining angle brackets from malformed tags are stripped.
        ...
        <div className="max-w-3xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between mb-3">
            <a
              ...
              className="text-black hover:text-gray-600 text-sm truncate max-w-xs"
            >
              ↗ Source
            </a>
